...
@implementation BGSignalExceptionHandler
...
+ (void)signalRegister {
    BGSignalRegister(SIGABRT);
    BGSignalRegister(SIGBUS);
    BGSignalRegister(SIGFPE);
    BGSignalRegister(SIGILL);
    BGSignalRegister(SIGPIPE);
    BGSignalRegister(SIGSEGV);
    BGSignalRegister(SIGSYS);
    BGSignalRegister(SIGTRAP);
}

#pragma mark - Private

#pragma mark Register Signal
static void BGSignalRegister(int signal) {
    struct sigaction action;
    action.sa_sigaction = BGSignalHandler;
...
static void BGSignalHandler(int signal, siginfo_t* info, void* context) {
    ...
    // 日志数据入库
    [BGEventStore.sharedInstance insertRecord:BGAnalyticsTypeCrash name:@"InternalCrash" parameters:@{@"Crash(Signal)":mstr}];
    
    BGClearSignalRigister();
...
#pragma mark Clear
static void BGClearSignalRigister() {
